...
def walk_folders(pathroot,new_suffix,files_path,files_stem,files_name):
    for element in pathroot.iterdir():
        if element.is_file():
            ...
            add_element(element,new_suffix,files_path,files_stem,files_name)
        else:
            print(f'{element} - папка')
            walk_folders(element,new_suffix,files_path,files_stem,files_name)
    ...
    if len(sys.argv) < 2:
        ...
        if pathroot.exists():
            if pathroot.is_dir():
                ...
                files_stem = []
                files_name = []
                walk_folders(pathroot,new_suffix,files_path,files_stem,files_name)
                        ...
                        for elements in set_of_key:
                            for names in files_name:
                                if elements in names:
                                    replace_file(pathroot,name,names)
                try:
                    directory_name = f'{pathroot}\{name}'
                    new_path = Path(directory_name)
                    Path.rmdir(new_path)
                ...
                for key, value in my_directory_files.items():
                    if key == 'архивы':
                        ...
                        for elements in set_of_key:
                            for names in files_name:
                                if elements in names:
                                    replace_file_archive(pathroot,name,names)
                try:
                    directory_name = f'{pathroot}\{name}'
                    new_path = Path(directory_name)
                    Path.rmdir(new_path)
